...
/*   Updated: 2019/03/30 22:11:20 by mhedeon          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
SDL_Color		lighting(t_rt *rt, t_fov pv, t_vec normal, t_object *obj)
{
	...
	while (rt->light != NULL)
	{
		...
		in += rt->light->type == PARALLEL ? point(rt, pv, normal, obj->specular, substruct(multiply(-(1000000), rt->light->normal), (t_vec){0,0,0})) : 0.0;
		if (rt->light->type == DIRECT && (spot_effect = dual_cone_spotlight(pv.cam, rt->light, rt->light->cos_angle)) != 0.0)
		{
			t = rt->light->type == DIRECT ? point(rt, pv, normal, obj->specular, substruct(rt->light->pos, pv.cam)) : 0.0;
			in += (spot_effect * t);
		}
